...
*/
    commandMsg match {
      ...
      case CommandMsg("frente", _, _) => board ! Forward
      case CommandMsg("tras", _, _) => board ! Reverse
      case CommandMsg("direita", _, _) => board ! Right
      case CommandMsg("esquerda", _, _) => board ! Left
      case CommandMsg("acelera", _, _) => board ! Acelerate
      case CommandMsg("freia", _, _) => board ! SlowDonw
      case CommandMsg("parar", _, _) => board ! Stop
